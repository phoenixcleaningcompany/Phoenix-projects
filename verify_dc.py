#!/usr/bin/env python3
"""verify_fs.py - QA for DC (Delivery & Courier Services) workwear pages.

Usage:  python3 verify_fs.py [folder]     default folder: outputs/
Needs:  DC_towns.csv in repo root (Rank, Town, Population, Nation)
...
WORD_MIN = 1200
LEAD = "polo"      # DC lead (embroidered polos)
CO_LEAD = "hi-vis"  # co-lead (hi-vis vests)
TRI_LEAD = "softshell" # third lead (softshell)
...
def load_towns():
    ...
    if not os.path.exists(path):
        ...
        path = cands[0]
        print(f"NOTE: DC_towns.csv not found, using {path}")
    towns = {}
...
def check_file(path, towns, seen_desc, seen_title):
    ...
    dead = sorted({s for s in re.findall(r'href=["\']dc-([a-z0-9-]+)\.html', txt, re.I)
...
def main():
    ...
    files = sorted(f for f in os.listdir(folder) if f.endswith(".html") and f.startswith("dc-"))
    if not files:
