#!/usr/bin/env python3
"""diag_dc.py - over-cap-pair diagnostic for the DC series.

When scodc_re.py reports a pair over the 52% cap, this isolates exactly what
...
Usage:
  python3 diag_dc.py                      # worst over-cap pair in outputs/
  python3 diag_dc.py outputs              # worst over-cap pair in a folder
  python3 diag_dc.py dc-a.html dc-b.html  # a specific pair (control auto-picked)
  python3 diag_dc.py dc-a.html dc-b.html dc-c.html   # explicit control = c
"""
import os, sys, glob
from itertools import combinations
import score_dc as S
...
def main():
    ...
    if len(files) >= 2:
        ...
        paths = collect(args)
        if len(paths) < 2:
            sys.exit("need at least 2 dc-*.html files")
