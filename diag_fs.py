#!/usr/bin/env python3
"""diag_fs.py - over-cap-pair diagnostic for the FS series.

When scofs_re.py reports a pair over the 52% cap, this isolates exactly what
...
Usage:
  python3 diag_fs.py                      # worst over-cap pair in outputs/
  python3 diag_fs.py outputs              # worst over-cap pair in a folder
  python3 diag_fs.py fs-a.html fs-b.html  # a specific pair (control auto-picked)
  python3 diag_fs.py fs-a.html fs-b.html fs-c.html   # explicit control = c
"""
import os, sys, glob
...
def main():
    ...
    if len(files) >= 2:
        ...
        paths = collect(args)
        if len(paths) < 2:
            sys.exit("need at least 2 fs-*.html files")
