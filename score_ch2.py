#!/usr/bin/env python3
"""scovt_re.py - sentence-level uniqueness scorer for CH2 (Charities, Non-Profits & Volunteers) workwear pages.

Usage:  python3 scovt_re.py outputs/ch2-*.html
        python3 scovt_re.py outputs            (folder = all ch2-*.html)
...
CHROME = ["ch2-header", "ch2-trust-strip", "ch2-stats", "ch2-jump-links",
          "ch2-cta-bar", "ch2-contact-block", "ch2-footer", "ch2-nearby",
          "ch2-contract", "ch2-product-grid"]
...
def strip(html):
    ...
    html = re.sub(r'<div class="ch2-illust-caption">.*?</div>', ' ', html, flags=re.I | re.S)
...
def town_of(path):
    ...
    m = re.match(r'ch2-(.+)\.html$', base, re.I)
...
def collect(args):
    ...
    for a in args:
        if os.path.isdir(a):
            paths += sorted(glob.glob(os.path.join(a, "ch2-*.html")))
        else:
...
def main():
    ...
    paths = collect(args)
    if len(paths) < 2:
        print(f"Need at least 2 ch2-*.html files to compare (found {len(paths)}).")
        if paths:
