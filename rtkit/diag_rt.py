#!/usr/bin/env python3
"""diag_rt.py - over-cap-pair diagnostic for the RT series.

When scort_re.py reports a pair over the 52% cap, this isolates exactly what
...
Usage:
  python3 diag_rt.py                      # worst over-cap pair in outputs/
  python3 diag_rt.py outputs              # worst over-cap pair in a folder
  python3 diag_rt.py rt-a.html rt-b.html  # a specific pair (control auto-picked)
  python3 diag_rt.py rt-a.html rt-b.html rt-c.html   # explicit control = c
"""
import os, sys, glob
from itertools import combinations
import score_rt as S
...
def main():
    ...
    if len(files) >= 2:
        ...
        paths = collect(args)
        if len(paths) < 2:
            sys.exit("need at least 2 rt-*.html files")
