#!/usr/bin/env python3
"""verify_fs.py - QA for RT (Supermarkets & Retail) workwear pages.

Usage:  python3 verify_fs.py [folder]     default folder: outputs/
Needs:  RT_towns.csv in repo root (Rank, Town, Population, Nation)
...
ballistic / stab-vest language, plus cleaning bleed (tunic, COSHH; NOTE tabard is legal - RT core),
...
BLEED = ["hairnet", "hygiene coat", "BRCGS", "HACCP", "high-care", "snood", "beard net",
         ...
         "COSHH", "colour-coded cleaning", "tunic",
...
def load_towns():
    ...
    if not os.path.exists(path):
        ...
        path = cands[0]
        print(f"NOTE: RT_towns.csv not found, using {path}")
    towns = {}
...
def check_file(path, towns, seen_desc, seen_title):
    ...
    dead = sorted({s for s in re.findall(r'href=["\']rt-([a-z0-9-]+)\.html', txt, re.I)
...
def main():
    ...
    files = sorted(f for f in os.listdir(folder) if f.endswith(".html") and f.startswith("rt-"))
    if not files:
