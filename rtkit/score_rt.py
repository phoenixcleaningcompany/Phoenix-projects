#!/usr/bin/env python3
"""scort_re.py - sentence-level uniqueness scorer for RT (Supermarkets & Retail) workwear pages.

Usage:  python3 scort_re.py outputs/rt-*.html
        python3 scort_re.py outputs            (folder = all rt-*.html)
...
CHROME = ["rt-header", "rt-trust-strip", "rt-stats", "rt-jump-links",
          "rt-cta-bar", "rt-contact-block", "rt-footer", "rt-nearby",
          "rt-contract", "rt-product-grid"]
...
def strip(html):
    ...
    html = re.sub(r'<div class="rt-illust-caption">.*?</div>', ' ', html, flags=re.I | re.S)
...
def town_of(path):
    ...
    m = re.match(r'rt-(.+)\.html$', base, re.I)
...
def collect(args):
    ...
    for a in args:
        if os.path.isdir(a):
            paths += sorted(glob.glob(os.path.join(a, "rt-*.html")))
        else:
...
def main():
    ...
    paths = collect(args)
    if len(paths) < 2:
        print(f"Need at least 2 rt-*.html files to compare (found {len(paths)}).")
        if paths:
