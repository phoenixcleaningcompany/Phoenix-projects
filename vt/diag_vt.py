#!/usr/bin/env python3
"""diag_vt.py - over-cap-pair diagnostic for the VT series.

When score_vt.py reports a pair over the 52% cap, this isolates exactly what
...
Usage:
  ...
  python3 diag_vt.py vt-a.html vt-b.html  # a specific pair (control auto-picked)
  python3 diag_vt.py vt-a.html vt-b.html vt-c.html   # explicit control = c
"""
import os, sys, glob
from itertools import combinations
import score_vt as S
...
def main():
    ...
    if len(files) >= 2:
        ...
        paths = collect(args)
        if len(paths) < 2:
            sys.exit("need at least 2 vt-*.html files")
