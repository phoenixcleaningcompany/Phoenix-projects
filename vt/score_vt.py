#!/usr/bin/env python3
"""scovt_re.py - sentence-level uniqueness scorer for VT (Veterinary & Animal Care) workwear pages.

Usage:  python3 scovt_re.py outputs/vt-*.html
        python3 scovt_re.py outputs            (folder = all vt-*.html)
...
CHROME = ["vt-header", "vt-trust-strip", "vt-stats", "vt-jump-links",
          "vt-cta-bar", "vt-contact-block", "vt-footer", "vt-nearby",
          "vt-contract", "vt-product-grid"]
...
def strip(html):
    ...
    html = re.sub(r'<div class="vt-illust-caption">.*?</div>', ' ', html, flags=re.I | re.S)
...
def town_of(path):
    ...
    m = re.match(r'vt-(.+)\.html$', base, re.I)
...
def collect(args):
    ...
    for a in args:
        if os.path.isdir(a):
            paths += sorted(glob.glob(os.path.join(a, "vt-*.html")))
        else:
...
def main():
    ...
    paths = collect(args)
    if len(paths) < 2:
        print(f"Need at least 2 vt-*.html files to compare (found {len(paths)}).")
        if paths:
