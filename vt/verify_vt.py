#!/usr/bin/env python3
"""verify_vt.py - QA for VT (Veterinary & Animal Care) workwear pages.

Usage:  python3 verify_vt.py [folder]     default folder: outputs/
Needs:  VT_towns.csv in repo root (Rank, Town, Population, Nation)
...
lead products (scrubs + polo + fleece), duplicate title/description, dead
...
(fleeces/waterproofs/safety shoes/wellingtons). scrubs, tunic, polo, fleece,
...
WORD_MIN = 1200
LEAD = "scrubs"   # VT lead (veterinary scrubs)
...
def load_towns():
    ...
    if not os.path.exists(path):
        ...
        path = cands[0]
        print(f"NOTE: VT_towns.csv not found, using {path}")
    towns = {}
...
def main():
    ...
    files = sorted(f for f in os.listdir(folder) if f.endswith(".html") and f.startswith("vt-"))
    if not files:
        sys.exit(f"ERROR: no vt-*.html files in {folder}/")
    towns = load_towns()
