#!/usr/bin/env python3
"""verify_hm.py - QA for HM (Highway Maintenance & Roadworks) workwear pages.

Usage:  python3 verify_hm.py [folder]     default folder: outputs/
Needs:  HM_towns.csv in repo root (Rank, Town, Population, Nation)
...
WORD_MIN = 1200
LEAD = "hi-vis"     # HM lead (Class 3 hi-vis)
CO_LEAD = "safety boots" # co-lead (safety boots)
TRI_LEAD = "waterproofs" # third lead (waterproofs)
...
BLEED = ["hairnet", "hygiene coat", "BRCGS", "HACCP", "high-care", "snood", "beard net",
         ...
         "COSHH", "colour-coded cleaning", "tabard", "tunic",
         "RCV", "refuse collection", "bin lorry", "kerbside collection", "HWRC",
         "solar panel", "EV charging", "EV charge", "heat pump", "MCS-certified", "MCS-registered", "fall protection",
         "Ofsted", "KCSIE", "Care Quality Commission", "CQC", "Care Inspectorate",
         "Gas Safe", "chainsaw", "arb PPE", "EN ISO 11393", "LANTRA", "NPTC", "tree surgery",
         "gym", "hoodie", "athleisure", "club crest", "matchday", "tracksuit"]
...
def load_towns():
    ...
    if not os.path.exists(path):
        ...
        path = cands[0]
        print(f"NOTE: HM_towns.csv not found, using {path}")
    towns = {}
...
def check_file(path, towns, seen_desc, seen_title):
    ...
    dead = sorted({s for s in re.findall(r'href=["\']hm-([a-z0-9-]+)\.html', txt, re.I)
...
def main():
    ...
    files = sorted(f for f in os.listdir(folder) if f.endswith(".html") and f.startswith("hm-"))
    if not files:
        sys.exit(f"ERROR: no se-*.html files in {folder}/")
    towns = load_towns()
