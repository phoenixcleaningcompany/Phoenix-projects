#!/usr/bin/env python3
"""scovt_re.py - sentence-level uniqueness scorer for TL (Travel, Tourism & Leisure) workwear pages.

Usage:  python3 scovt_re.py outputs/tl-*.html
        python3 scovt_re.py outputs            (folder = all tl-*.html)
...
CHROME = ["tl-header", "tl-trust-strip", "tl-stats", "tl-jump-links",
          "tl-cta-bar", "tl-contact-block", "tl-footer", "tl-nearby",
          "tl-contract", "tl-product-grid"]
...
def strip(html):
    ...
    html = re.sub(r'<div class="tl-illust-caption">.*?</div>', ' ', html, flags=re.I | re.S)
...
def town_of(path):
    ...
    m = re.match(r'tl-(.+)\.html$', base, re.I)
...
def collect(args):
    ...
    for a in args:
        if os.path.isdir(a):
            paths += sorted(glob.glob(os.path.join(a, "tl-*.html")))
        else:
...
def main():
    ...
    paths = collect(args)
    if len(paths) < 2:
        print(f"Need at least 2 tl-*.html files to compare (found {len(paths)}).")
        if paths:
