#!/usr/bin/env python3
"""scohm_re.py - sentence-level uniqueness scorer for HM (Highway Maintenance & Roadworks) workwear pages.

Usage:  python3 scohm_re.py outputs/hm-*.html
        python3 scohm_re.py outputs            (folder = all hm-*.html)
...
def strip(html):
    ...
    html = re.sub(r'<div class="hm-illust-caption">.*?</div>', ' ', html, flags=re.I | re.S)
...
def town_of(path):
    ...
    m = re.match(r'hm-(.+)\.html$', base, re.I)
...
def main():
    ...
    paths = collect(args)
    if len(paths) < 2:
        print(f"Need at least 2 hm-*.html files to compare (found {len(paths)}).")
        if paths:
