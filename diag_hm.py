#!/usr/bin/env python3
"""diag_hm.py - over-cap-pair diagnostic for the HM series.

When scohm_re.py reports a pair over the 52% cap, this isolates exactly what
...
Usage:
  python3 diag_hm.py                      # worst over-cap pair in outputs/
  python3 diag_hm.py outputs              # worst over-cap pair in a folder
  python3 diag_hm.py hm-a.html hm-b.html  # a specific pair (control auto-picked)
  python3 diag_hm.py hm-a.html hm-b.html hm-c.html   # explicit control = c
"""
import os, sys, glob
from itertools import combinations
import score_hm as S
...
def collect(args):
    ...
    for a in args:
        if os.path.isdir(a):
            paths += sorted(glob.glob(os.path.join(a, "hm-*.html")))
...
def main():
    ...
    if len(files) >= 2:
        ...
        paths = collect(args)
        if len(paths) < 2:
            sys.exit("need at least 2 hm-*.html files")
