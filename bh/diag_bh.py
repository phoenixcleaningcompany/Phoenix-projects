#!/usr/bin/env python3
"""diag_bh.py - over-cap-pair diagnostic for the BH series.

When scobh_re.py reports a pair over the 52% cap, this isolates exactly what
...
Usage:
  python3 diag_bh.py                      # worst over-cap pair in outputs/
  python3 diag_bh.py outputs              # worst over-cap pair in a folder
  python3 diag_bh.py bh-a.html bh-b.html  # a specific pair (control auto-picked)
  python3 diag_bh.py bh-a.html bh-b.html bh-c.html   # explicit control = c
"""
import os, sys, glob
from itertools import combinations
import score_bh as S
...
def main():
    ...
    if len(files) >= 2:
        ...
        paths = collect(args)
        if len(paths) < 2:
            sys.exit("need at least 2 bh-*.html files")
