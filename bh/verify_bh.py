#!/usr/bin/env python3
"""verify_bh.py - QA for BH (Beauty, Hair & Spa) workwear pages.

Usage:  python3 verify_bh.py [folder]     default folder: outputs/
Needs:  BH_towns.csv in repo root (Rank, Town, Population, Nation)
...
claims, cross-series bleed, lead products (tunic + polo + apron),
...
ballistic / stab-vest language, plus cleaning bleed (COSHH; NOTE tunic is legal - BH core; tabard/retail/courier terms blocked),
...
WORD_MIN = 1200
LEAD = "tunic"    # BH lead (tunics)
CO_LEAD = "polo"   # co-lead (polos)
TRI_LEAD = "apron" # third lead (aprons)
...
BLEED = ["hairnet", "hygiene coat", "BRCGS", "HACCP", "high-care", "snood", "beard net",
         ...
         "softshell", "multidrop", "owner-driver", "courier"]
...
def load_towns():
    ...
    if not os.path.exists(path):
        ...
        path = cands[0]
        print(f"NOTE: BH_towns.csv not found, using {path}")
    towns = {}
...
def check_file(path, towns, seen_desc, seen_title):
    ...
    dead = sorted({s for s in re.findall(r'href=["\']bh-([a-z0-9-]+)\.html', txt, re.I)
...
def main():
    ...
    files = sorted(f for f in os.listdir(folder) if f.endswith(".html") and f.startswith("bh-"))
    if not files:
        sys.exit(f"ERROR: no bh-*.html files in {folder}/")
    towns = load_towns()
