#!/usr/bin/env python3
"""scobh_re.py - sentence-level uniqueness scorer for BH (Beauty, Hair & Spa) workwear pages.

Usage:  python3 scobh_re.py outputs/bh-*.html
        python3 scobh_re.py outputs            (folder = all bh-*.html)
...
CHROME = ["bh-header", "bh-trust-strip", "bh-stats", "bh-jump-links",
          "bh-cta-bar", "bh-contact-block", "bh-footer", "bh-nearby",
          "bh-contract", "bh-product-grid"]
...
def strip(html):
    ...
    html = re.sub(r'<div class="bh-illust-caption">.*?</div>', ' ', html, flags=re.I | re.S)
...
def town_of(path):
    ...
    m = re.match(r'bh-(.+)\.html$', base, re.I)
...
def collect(args):
    ...
    for a in args:
        if os.path.isdir(a):
            paths += sorted(glob.glob(os.path.join(a, "bh-*.html")))
        else:
...
def main():
    ...
    paths = collect(args)
    if len(paths) < 2:
        print(f"Need at least 2 bh-*.html files to compare (found {len(paths)}).")
        if paths:
