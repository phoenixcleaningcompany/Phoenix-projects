#!/usr/bin/env python3
"""verify_fs.py - QA for FS (Forestry, Arboriculture & Tree Surgery) workwear pages.
...
WORD_MIN = 1200
LEAD = "chainsaw"   # FS lead (chainsaw trousers/PPE)
CO_LEAD = "safety boots" # co-lead (chainsaw/safety boots)
TRI_LEAD = "helmet"   # third lead (forestry helmet)
...
def load_towns():
    ...
    if not os.path.exists(path):
        ...
        path = cands[0]
        print(f"NOTE: FS_towns.csv not found, using {path}")
    towns = {}
...
def check_file(path, towns, seen_desc, seen_title):
    ...
    dead = sorted({s for s in re.findall(r'href=["\']fs-([a-z0-9-]+)\.html', txt, re.I)
...
def main():
    ...
    files = sorted(f for f in os.listdir(folder) if f.endswith(".html") and f.startswith("fs-"))
    if not files:
