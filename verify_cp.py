#!/usr/bin/env python3
"""verify_cp.py - QA for CP (Corporate & Professional Services) workwear pages.

Usage:  python3 verify_vt.py [folder]     default folder: outputs/
Needs:  CP_towns.csv in repo root (Rank, Town, Population, Nation)
...
lead products (polo + softshell + fleece), duplicate title/description, dead
...
(fleeces/waterproofs/safety shoes/wellingtons). polo, shirt, softshell, fleece, embroidery,
...
CO_LEAD = "softshell"   # co-lead (softshells)
TRI_LEAD = "fleece" # third lead (fleeces)
...
BLEED = ["hairnet", "hygiene coat", "BRCGS", "HACCP", "high-care", "snood", "beard net",
         ...
         "tourism", "theme park", "holiday park", "activity centre", "tour operator",
         "festival", "stage", "crew", "rigging", "front-of-house", "steward", "concert",
...
def load_towns():
    ...
    if not os.path.exists(path):
        ...
        path = cands[0]
        print(f"NOTE: CP_towns.csv not found, using {path}")
    towns = {}
...
def check_file(path, towns, seen_desc, seen_title):
    ...
    dead = sorted({s for s in re.findall(r'href=["\']cp-([a-z0-9-]+)\.html', txt, re.I)
...
def main():
    ...
    files = sorted(f for f in os.listdir(folder) if f.endswith(".html") and f.startswith("cp-"))
    if not files:
        sys.exit(f"ERROR: no cp-*.html files in {folder}/")
    towns = load_towns()
