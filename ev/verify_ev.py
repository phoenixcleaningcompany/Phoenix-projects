#!/usr/bin/env python3
"""verify_ev.py - QA for EV (Entertainment, Events & Festivals) workwear pages.

Usage:  python3 verify_vt.py [folder]     default folder: outputs/
Needs:  EV_towns.csv in repo root (Rank, Town, Population, Nation)
...
lead products (polo + hi-vis + fleece), duplicate title/description, dead
...
(fleeces/waterproofs/safety shoes/wellingtons). polo, hi-vis, fleece, waterproof, safety boots,
...
BLEED = ["hairnet", "hygiene coat", "BRCGS", "HACCP", "high-care", "snood", "beard net",
         ...
         "softshell", "tourism", "theme park", "holiday park", "activity centre", "tour operator",
...
def load_towns():
    ...
    if not os.path.exists(path):
        ...
        path = cands[0]
        print(f"NOTE: EV_towns.csv not found, using {path}")
    towns = {}
...
def check_file(path, towns, seen_desc, seen_title):
    ...
    dead = sorted({s for s in re.findall(r'href=["\']ev-([a-z0-9-]+)\.html', txt, re.I)
...
def main():
    ...
    files = sorted(f for f in os.listdir(folder) if f.endswith(".html") and f.startswith("ev-"))
    if not files:
        sys.exit(f"ERROR: no ev-*.html files in {folder}/")
    towns = load_towns()
