#!/usr/bin/env python3
"""scovt_re.py - sentence-level uniqueness scorer for EV (Entertainment, Events & Festivals) workwear pages.

Usage:  python3 scovt_re.py outputs/ev-*.html
        python3 scovt_re.py outputs            (folder = all ev-*.html)
...
CHROME = ["ev-header", "ev-trust-strip", "ev-stats", "ev-jump-links",
          "ev-cta-bar", "ev-contact-block", "ev-footer", "ev-nearby",
          "ev-contract", "ev-product-grid"]
...
def strip(html):
    ...
    html = re.sub(r'<div class="ev-illust-caption">.*?</div>', ' ', html, flags=re.I | re.S)
...
def town_of(path):
    ...
    m = re.match(r'ev-(.+)\.html$', base, re.I)
...
def collect(args):
    ...
    for a in args:
        if os.path.isdir(a):
            paths += sorted(glob.glob(os.path.join(a, "ev-*.html")))
        else:
...
def main():
    ...
    paths = collect(args)
    if len(paths) < 2:
        print(f"Need at least 2 ev-*.html files to compare (found {len(paths)}).")
        if paths:
