#!/usr/bin/env python3
"""diag_vt.py - over-cap-pair diagnostic for the EV series.

When score_ev.py reports a pair over the 52% cap, this isolates exactly what
...
Usage:
  ...
  python3 diag_vt.py ev-a.html ev-b.html  # a specific pair (control auto-picked)
  python3 diag_vt.py ev-a.html ev-b.html ev-c.html   # explicit control = c
"""
import os, sys, glob
from itertools import combinations
import score_ev as S
...
def main():
    ...
    if len(files) >= 2:
        ...
        paths = collect(args)
        if len(paths) < 2:
            sys.exit("need at least 2 ev-*.html files")
