#!/usr/bin/env python3
"""scovt_re.py - sentence-level uniqueness scorer for TC (Telecoms & Network Installation) workwear pages.

Usage:  python3 scovt_re.py outputs/tc-*.html
        python3 scovt_re.py outputs            (folder = all tc-*.html)
...
CHROME = ["tc-header", "tc-trust-strip", "tc-stats", "tc-jump-links",
          "tc-cta-bar", "tc-contact-block", "tc-footer", "tc-nearby",
          "tc-contract", "tc-product-grid"]
...
def strip(html):
    ...
    html = re.sub(r'<div class="tc-illust-caption">.*?</div>', ' ', html, flags=re.I | re.S)
...
def town_of(path):
    ...
    m = re.match(r'tc-(.+)\.html$', base, re.I)
...
def main():
    ...
    paths = collect(args)
    if len(paths) < 2:
        print(f"Need at least 2 tc-*.html files to compare (found {len(paths)}).")
        if paths:
