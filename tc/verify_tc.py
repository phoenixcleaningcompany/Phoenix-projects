#!/usr/bin/env python3
"""verify_tc.py - QA for TC (Telecoms & Network Installation) workwear pages.

Usage:  python3 verify_vt.py [folder]     default folder: outputs/
Needs:  TC_towns.csv in repo root (Rank, Town, Population, Nation)
...
lead products (polo + hi-vis + softshell), duplicate title/description, dead
...
(fleeces/waterproofs/safety shoes/wellingtons). polo, hi-vis, softshell, cargo, fleece,
...
def load_towns():
    ...
    if not os.path.exists(path):
        ...
        path = cands[0]
        print(f"NOTE: TC_towns.csv not found, using {path}")
    towns = {}
...
def check_file(path, towns, seen_desc, seen_title):
    ...
    dead = sorted({s for s in re.findall(r'href=["\']tc-([a-z0-9-]+)\.html', txt, re.I)
...
def main():
    ...
    files = sorted(f for f in os.listdir(folder) if f.endswith(".html") and f.startswith("tc-"))
    if not files:
        sys.exit(f"ERROR: no tc-*.html files in {folder}/")
    towns = load_towns()
