#!/usr/bin/env python3
"""verify_ch2.py - QA for CH2 (Charities, Non-Profits & Volunteers) workwear pages.

Usage:  python3 verify_vt.py [folder]     default folder: outputs/
Needs:  CH2_towns.csv in repo root (Rank, Town, Population, Nation)
...
(fleeces/waterproofs/safety shoes/wellingtons). polo, hi-vis, fleece, waterproof, apron,
...
BLEED = ["hairnet", "hygiene coat", "BRCGS", "HACCP", "high-care", "snood", "beard net",
         ...
         "softshell", "telecoms", "fibre", "broadband", "Openreach", "street cabinet",
...
def load_towns():
    ...
    if not os.path.exists(path):
        ...
        path = cands[0]
        print(f"NOTE: CH2_towns.csv not found, using {path}")
    towns = {}
...
def check_file(path, towns, seen_desc, seen_title):
    ...
    dead = sorted({s for s in re.findall(r'href=["\']ch2-([a-z0-9-]+)\.html', txt, re.I)
...
def main():
    ...
    files = sorted(f for f in os.listdir(folder) if f.endswith(".html") and f.startswith("ch2-"))
    if not files:
        sys.exit(f"ERROR: no ch2-*.html files in {folder}/")
    towns = load_towns()
