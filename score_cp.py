#!/usr/bin/env python3
"""scovt_re.py - sentence-level uniqueness scorer for CP (Corporate & Professional Services) workwear pages.

Usage:  python3 scovt_re.py outputs/cp-*.html
        python3 scovt_re.py outputs            (folder = all cp-*.html)
...
CHROME = ["cp-header", "cp-trust-strip", "cp-stats", "cp-jump-links",
          "cp-cta-bar", "cp-contact-block", "cp-footer", "cp-nearby",
          "cp-contract", "cp-product-grid"]
...
def strip(html):
    ...
    html = re.sub(r'<div class="cp-illust-caption">.*?</div>', ' ', html, flags=re.I | re.S)
...
def town_of(path):
    ...
    m = re.match(r'cp-(.+)\.html$', base, re.I)
...
def collect(args):
    ...
    for a in args:
        if os.path.isdir(a):
            paths += sorted(glob.glob(os.path.join(a, "cp-*.html")))
        else:
...
def main():
    ...
    paths = collect(args)
    if len(paths) < 2:
        print(f"Need at least 2 cp-*.html files to compare (found {len(paths)}).")
        if paths:
