#!/usr/bin/env python3
"""scodc_re.py - sentence-level uniqueness scorer for DC (Delivery & Courier Services) workwear pages.

Usage:  python3 scodc_re.py outputs/dc-*.html
        python3 scodc_re.py outputs            (folder = all dc-*.html)
...
CHROME = ["dc-header", "dc-trust-strip", "dc-stats", "dc-jump-links",
          "dc-cta-bar", "dc-contact-block", "dc-footer", "dc-nearby",
          "dc-contract", "dc-product-grid"]
...
def strip(html):
    ...
    html = re.sub(r'<div class="dc-illust-caption">.*?</div>', ' ', html, flags=re.I | re.S)
...
def town_of(path):
    ...
    m = re.match(r'dc-(.+)\.html$', base, re.I)
...
def collect(args):
    ...
    for a in args:
        if os.path.isdir(a):
            paths += sorted(glob.glob(os.path.join(a, "dc-*.html")))
        else:
...
def main():
    ...
    paths = collect(args)
    if len(paths) < 2:
        print(f"Need at least 2 dc-*.html files to compare (found {len(paths)}).")
        if paths:
