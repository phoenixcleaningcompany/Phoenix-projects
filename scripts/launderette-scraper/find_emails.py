#!/usr/bin/env python3
"""
Same approach as the other scrapers: visits each launderette's website
(home, /contact, /contact-us) and pulls an email from mailto: links or
plain-text patterns. Short timeout, stops trying further pages on a
...
SOURCE = "launderettes_independent.csv"
...
def auto_commit(out_path, processed_count):
    ...
    try:
        ...
        subprocess.run(
            ["git", "commit", "-m", f"Auto-checkpoint: {processed_count} launderette sites processed"],
...
def main():
    ...
    fieldnames = ["Country", "Town", "Name", "Address", "Phone", "Website", "Email", "Found on page"]
    ...
    with open(args.out, mode, newline="", encoding="utf-8") as f:
        ...
        for i, r in enumerate(rows, 1):
            ...
            writer.writerow({
                "Country": r["Country"], "Town": r["Town"], "Name": r["Name"], "Address": r["Address"],
