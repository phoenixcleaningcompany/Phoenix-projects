...
by lowercased email address (the same engineer sometimes surfaces under both
query phrasings, or under a neighbouring town search), merges site names for
an email shared across multiple listed sites.
"""
import csv
...
def site_label(sites):
    ...
    return f"{sites[0]} and {len(sites) - 1} other catering engineers"
...
def main():
    ...
    with open(OUT, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=["Email Address", "SITE", "PHONE", "TOWN", "SITECOUNT"])
        writer.writeheader()
