# Wales Cold Outreach — 3-Email Sequence (v3)
...
Positioned around the two main services for this vertical: **TR19 kitchen extraction cleaning and laundry duct cleaning** (LEV testing dropped from this sequence — kept available if a prospect asks, but not part of the pitch here).

Subject lines keep the `*|SITE|*` Mailchimp merge tag. Body text uses "your site" throughout instead, so nothing needs manual editing per recipient if sending without a merge tool — only the subject personalizes.
...
The process: survey (inspect and agree scope based on how hard the kitchen runs), strip (filters and panels removed, cookline protected), degrease (canopy to fan, by hand, with before/after photos at each section), certify (dated certificate plus a next-due date for the fire logbook).
...
The same logic applies to laundry extraction ducts if you run an on-site laundry — lint builds up the same way grease does, it's just as hidden, and it's the same fire-risk mechanism.

If you're not sure which band your site's in, or when either system was last done properly, happy to take a look — no obligation.
...
3. Kitchen fire suppression systems (wet chemical/Ansul-type) overdue their required annual service, hidden inside the canopy
...
Numbers 1 and 2 are where I can actually help — TR19 kitchen extraction cleaning and laundry duct cleaning, done properly, with paperwork that holds up. If either is worth checking at your site, I'm easy to find: 07961 915018 or officeductclean@gmail.com. No pressure either way.
...
## Notes
- `*|SITE|*` only appears in subjects now — bodies use "your site" so nothing needs manual substitution if you're not running these through Mailchimp/a merge tool.
- LEV testing is deliberately absent from this sequence per your call — extraction and laundry duct cleaning are the two services being pitched to this vertical. If a prospect asks about LEV, that's a natural in-person/reply-thread mention, just not part of the cold pitch.
- Item 3 in Email 3 was swapped from LEV/COSHH testing to fire suppression system servicing — a genuine, separate compliance item (not something Phoenix does), kept to preserve "ten things" without reintroducing LEV.
- Don't use the McDonald's/AWS/BaxterStorey "trusted by" claims here — confirmed genuine, but they belong on warm/trust-stage material, not a cold-open.
